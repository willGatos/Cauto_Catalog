...
const TabFilters = () => {
              ...
              <Popover.Panel className="absolute z-40 w-screen max-w-sm px-4 mt-3 left-0 sm:px-0 lg:max-w-md">
                <div className="overflow-hidden rounded-2xl shadow-xl bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-700">
                  <div className="relative flex flex-col px-5 py-6 space-y-5">
                    <Checkbox
                      name="All Categorías"
                      label="All Categorías"
                      defaultChecked={categoriesState.includes(
                        "All Categorías"
                      )}
                      onChange={(checked) =>
                        handleChangeCategories(checked, "All Categorías")
                      }
                    />
  ...
  // OK
  const renderTabsSize = (attribute, values, key) => {
    return (
  ...
  const handleChecked = (checked: boolean, name: string, setAction) => {
    checked
      ? setAction((prev) => [...prev, name])
      : setAction((prev) => prev.filter((i) => i !== name));
  };
  // OK
  const renderMoreFilterItem = (name) => {
    console.log(attributesSelected, categoriesSelected);
        ...
        <div className="flex flex-col space-y-5">
          {name.map((item) => (
            <Checkbox
              ...
              label={item.value}
              onChange={(value) =>
                handleChecked(value, item.value, setAttributesSelected)
              }
  ...
  // OK
  const renderMoreFilterCategorie = (names) => {
    console.log(attributesSelected, categoriesSelected);
    // const list1 = data.filter((_, i) => i < data.length / 2);
    // const list2 = data.filter((_, i) => i >= data.length / 2);
    return (
        ...
        <div className="flex flex-col space-y-5">
          {names.map((item) => (
            <Checkbox
              ...
              label={item.name}
              onChange={(value) =>
                handleChecked(value, item.id, setCategoriesSelected)
              }
              defaultChecked={!!item.defaultChecked}
            />
          ))}
                    ...
                    <div className="px-6 sm:px-8 md:px-10 divide-y divide-neutral-200 dark:divide-neutral-800">
                      ...
                      <div className="py-7">
                        ...
                        <div className="mt-6 relative ">
                          {renderMoreFilterCategorie(categories)}
                        </div>
                      </div>
                      ...
                      {attributes.map((att) => (
                        <div className="py-7">
                          <h3 className="text-xl font-medium">{att.name}</h3>
                          <div className="mt-6 relative ">
                            {renderMoreFilterItem(att.value)}
                          </div>
                        </div>
                      ))}
  ...
  return (
    <div className=" lg:space-x-4">
      ...
      <div className="hidden lg:flex flex-1 space-x-4">
        ...
        {/*renderTabsColor()*/}
        {attributes.map((att, key) => renderTabsSize(att.name, att.value, key))}
        {}
        {/*renderTabIsOnsale()*/}
      ...
      {/* FOR RESPONSIVE MOBILE */}
      <div className=" overflow-x-auto lg:hidden space-x-4">
